...
from build_publishable_catalog import BROAD_GENRES, broad_genre
...
def normalized_word_sequence(value: str) -> list[str]:
    return re.findall(r"[a-zà-ÿ0-9]+", value.casefold())


def significant_word_sequence(value: str, language: str) -> list[str]:
    return [
        token
        for token in re.findall(r"[^\W_]+", value.casefold(), flags=re.UNICODE)
        if len(token) > 3 and token not in STOPWORDS[language]
    ]
...
with (DATA / "editorial-openings.tsv").open("r", encoding="utf-8", newline="") as handle:
    approved_openings = {
        row["id"].strip(): {language: row[language].strip() for language in LANGUAGES}
        for row in csv.DictReader(handle, delimiter="\t")
    }
...
if legacy_reason_compat != []:
    errors.append("Legacy essential-reasons compatibility file must stay empty")
...
analysis_openers = {
    (language, record_type): Counter()
    for language in LANGUAGES
    for record_type in ("album", "song")
}
for record in records:
    record_id = record["id"]
    master_record = master_records.get(record_id)
    if not master_record:
        errors.append(f"{record_id}: missing from master catalog")
    else:
        expected_genre = broad_genre(master_record["genre"])
        if record.get("genreDetail") != master_record["genre"]:
            errors.append(f"{record_id}: runtime detailed genre differs from master")
        if record.get("genre") != expected_genre:
            errors.append(
                f"{record_id}: runtime genre {record.get('genre')!r} should be {expected_genre!r}"
            )
    if record.get("genre") not in BROAD_GENRES:
        errors.append(f"{record_id}: unknown runtime navigation genre")
    ...
    for language in LANGUAGES:
        ...
        if not content.get("review") or not content.get("listenFor") or not content.get("entryPoint", {}).get("title"):
            ...
            review_paragraphs = content["review"]
            if len(review_paragraphs) < 2:
                errors.append(f"{record_id}: {language} editorial note has fewer than two paragraphs")
            if review_paragraphs and review_paragraphs[0] != approved_openings.get(record_id, {}).get(language):
                errors.append(f"{record_id}: {language} editorial note does not use its approved opening")
            if len(review_text.split()) < 65:
                errors.append(f"{record_id}: {language} editorial note is too short")
            if review_paragraphs:
                opening = review_paragraphs[0].lstrip()
                if re.match(re.escape(record["title"]), opening, flags=re.IGNORECASE):
                    errors.append(f"{record_id}: {language} editorial note repeats its title at the opening")
            title_without_parenthetical = re.sub(
                r"\s*\([^)]*\)\s*$",
                "",
                record["title"],
            )
            title_words = normalized_word_sequence(title_without_parenthetical)
            for paragraph_index, paragraph in enumerate(review_paragraphs[1:], start=2):
                if language == "es" and paragraph.startswith(("La canción ", "El disco ")):
                    errors.append(
                        f"{record_id}: {language} paragraph {paragraph_index} starts with a generic work label"
                    )
                paragraph_words = normalized_word_sequence(paragraph)
                if title_words and paragraph_words[:len(title_words)] == title_words:
                    errors.append(
                        f"{record_id}: {language} paragraph {paragraph_index} repeats the title at its opening"
                    )
            if len(review_paragraphs) > 1:
                first_lead = significant_word_sequence(
                    review_paragraphs[0],
                    language,
                )
                second_lead = significant_word_sequence(
                    review_paragraphs[1],
                    language,
                )
                if first_lead and second_lead and first_lead[0] == second_lead[0]:
                    errors.append(
                        f"{record_id}: {language} consecutive paragraphs repeat "
                        f"the same lead word ({first_lead[0]})"
                    )
                opener_words = normalized_word_sequence(review_paragraphs[1])[:2]
                analysis_openers[(language, record["type"])][" ".join(opener_words)] += 1
            if len(review_paragraphs) > 1:
                opening_ratio, opening_shared = overlap_ratio(
                    scrub_titles(review_paragraphs[0], record, notes[record_id]),
                    scrub_titles(" ".join(review_paragraphs[1:]), record, notes[record_id]),
                    language,
                )
                if opening_ratio >= 0.60 and len(opening_shared) >= 3:
                    errors.append(
                        f"{record_id}: {language} contextual opening overlaps sonic analysis "
                        f"({opening_ratio:.0%}: {', '.join(sorted(opening_shared))})"
                    )
...
for language in LANGUAGES:
    ...
    for record_type in ("album", "song"):
        repeated_openers = [
            (opener, count)
            for opener, count in analysis_openers[(language, record_type)].items()
            if opener and count > 35
        ]
        if repeated_openers:
            errors.append(
                f"Formulaic {language} {record_type} analysis openings detected: {repeated_openers}"
            )
