...
def resolve_sources(records: list[dict]) -> dict[str, str]:
    ...
    search_cache: dict[tuple[str, str, int], str] = {}
    ...
    for record in records:
        ...
        key = (
            normalized(record.get("coverSearchArtist") or record["artist"]),
            normalized(record.get("coverSearchTitle") or record["title"]),
            int(record.get("coverSearchYear") or record["year"]),
        )
        if key in search_cache:
            print(f"Resolviendo {record['id']}: reutilizando coincidencia")
