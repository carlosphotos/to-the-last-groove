...
# The catalog stores a precise editorial genre and exposes a smaller set of
# navigation genres. Hybrids whose first useful home cannot be inferred from
# token order are deliberately listed here. This also keeps a work such as
# Remain in Light from being filed under Soul merely because "funk" appears in
# its detailed genre.
BROAD_GENRE_OVERRIDES = {
    "Art rock y funk": "rock",
    "Blues": "blues",
    "Folk soul": "folk",
    "Funk electrónico": "electronic",
    "Funk rock": "rock",
    "Indie pop": "pop",
    "Morna": "global",
    "MPB y jazz": "latin",
    "R&B y hip-hop": "soul",
    "Reguetón y hip-hop": "latin",
    "Zamba argentina": "latin",
}
...
def load_editorial_card_copy() -> dict[str, dict[str, str]]:
    """Load an independent, localized teaser for every recommendation card."""
    path = DATA / "editorial-card-copy.tsv"
    with path.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle, delimiter="\t"))

    copy_by_id: dict[str, dict[str, str]] = {}
    for row_number, row in enumerate(rows, start=2):
        record_id = row.get("id", "").strip()
        if not record_id or record_id in copy_by_id:
            raise SystemExit(
                f"Missing or repeated card-copy id in {path.name}, row {row_number}"
            )
        localized = {
        ...
        if not all(localized.values()):
            raise SystemExit(f"Incomplete card copy for {record_id}")
        copy_by_id[record_id] = localized
    return copy_by_id


def load_editorial_openings() -> dict[str, dict[str, str]]:
    """Load a distinct opening paragraph for every listening note."""
    path = DATA / "editorial-openings.tsv"
    with path.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle, delimiter="\t"))
    ...
    for row_number, row in enumerate(rows, start=2):
        record_id = row.get("id", "").strip()
        if not record_id or record_id in openings:
            raise SystemExit(
                f"Missing or repeated editorial-opening id in {path.name}, row {row_number}"
            )
        localized = {
            language: row.get(language, "").strip()
            for language in LANGUAGES
        ...
        if not all(localized.values()):
            raise SystemExit(f"Incomplete editorial opening for {record_id}")
        openings[record_id] = localized
    return openings
...
def broad_genre(genre: str) -> str:
    if genre in BROAD_GENRE_OVERRIDES:
        return BROAD_GENRE_OVERRIDES[genre]

    value = genre.casefold()
    if "jazz" in value:
        broad = "jazz"
    elif any(token in value for token in ("hip-hop", "hip hop")):
        broad = "hip-hop"
    elif any(token in value for token in ("soul", "r&b", "funk", "p-funk")):
        broad = "soul"
    elif any(token in value for token in ("electr", "ambient", "house", "downtempo", "trip-hop", "garage", "dance")):
        broad = "electronic"
    elif any(token in value for token in ("mpb", "bossa", "samba", "salsa", "tropic", "merengue", "reguet", "flamenco", "zamba")):
        broad = "latin"
    elif any(token in value for token in ("folk", "country", "chanson", "canción", "nueva canción")):
        broad = "folk"
    elif any(token in value for token in ("afro", "qawwali", "reggae", "árabe", "morna")):
        broad = "global"
    elif any(token in value for token in ("rock", "punk", "metal", "shoegaze", "kraut", "britpop", "dream pop", "indie")):
        broad = "rock"
    elif "pop" in value:
        broad = "pop"
    else:
        raise ValueError(
            f"Detailed genre {genre!r} has no explicit navigation mapping. "
            "Add it to the taxonomy before publishing."
        )

    if broad not in BROAD_GENRES:
        raise ValueError(f"Invalid navigation genre {broad!r} for {genre!r}")
    return broad


def profile_key(genre: str) -> str:
    key = broad_genre(genre)
    return {
        "jazz": "jazz", "blues": "soul", "soul": "soul",
        "hip-hop": "hiphop", "electronic": "electronic",
        "latin": "latin", "folk": "folk", "global": "global",
        "rock": "rock", "pop": "pop",
    }[key]
    ...
    for language in LANGUAGES:
        ...
        if len(opening_words) > 1 and len(set(opening_words)) == 1:
            for index in range(1, len(bullets)):
                ...
                for opening, replacement in opening_map.items():
                    if bullet.startswith(opening):
                        bullet = replacement + bullet[len(opening):]
...
def remove_editorial_colons(record: dict, editorial: dict) -> dict:
    """Remove colons used as prose hinges while preserving official titles."""
    protected_titles = [record["title"], *(editorial.get("trackMentions") or [])]

    def clean(value: str) -> str:
        placeholders: dict[str, str] = {}
        result = value
        for index, title in enumerate(sorted(set(protected_titles), key=len, reverse=True)):
            if not title:
                continue
            placeholder = f"⟦TITLE{index}⟧"
            result = result.replace(title, placeholder)
            placeholders[placeholder] = title

        transition_pattern = re.compile(
            r"\b(Después|Por último|Next|Finally|Ensuite|Enfin)\s*[:,]\s*"
            r"(Los|Las|Les|The|El|La|Le|L’)?\s*",
            flags=re.IGNORECASE,
        )

        def clean_transition(match: re.Match) -> str:
            article = match.group(2) or ""
            return f"{article[:1].upper() + article[1:]}{' ' if article and article != 'L’' else ''}"

        result = transition_pattern.sub(clean_transition, result)
        result = re.sub(r"\bLe s (?=[a-zà-ÿ])", "Les ", result)
        result = re.sub(r"\bLa s (?=[a-zá-úüñ])", "Las ", result)

        def sentence_break(match: re.Match) -> str:
            following = match.group(1)
            return ". " + following.upper()

        result = re.sub(r"\s*:\s+([A-Za-zÀ-ÿ])", sentence_break, result)
    ...
    for language in LANGUAGES:
        content = editorial.get(language) or {}
        content["review"] = [clean(item) for item in content.get("review", [])]
        content["listenFor"] = [clean(item) for item in content.get("listenFor", [])]
        entry_point = content.get("entryPoint") or {}
        entry_point["reason"] = clean(entry_point.get("reason", ""))
    return editorial


def replace_leading_title(record: dict, editorial: dict) -> dict:
    """Remove a repeated title without replacing it with one visible template."""
    neutral_subjects = {
        "album": {
            "en": ("This record", "The work", "The album", "This recording"),
            "fr": ("Cet enregistrement", "L’œuvre", "L’album", "Le disque"),
        },
        "song": {
            "en": ("This recording", "The piece", "The track", "The performance"),
            "fr": ("Cet enregistrement", "La pièce", "Le morceau", "L’interprétation"),
        },
    }
    generic_subjects = {
        "es": ("El disco", "La canción"),
        "en": ("The record", "The song"),
        "fr": ("L’album", "Le morceau"),
    }
    title_values = [record["title"]]
    short_title = re.sub(r"\s*\([^)]*\)\s*$", "", record["title"]).strip()
    if short_title and short_title != record["title"]:
        title_values.append(short_title)
    title_patterns = [
        re.sub(r"['’]", "['’]", re.escape(value))
    ...
    def replace_subject(predicate: str, language: str) -> str:
        if language == "es":
            return upper_first(predicate)
        subjects = neutral_subjects[record["type"]][language]
        subject = subjects[number(record["id"]) % len(subjects)]
        return f"{subject} {predicate.lstrip()}"

    for language in LANGUAGES:
        review = editorial.get(language, {}).get("review") or []
        if not review:
            continue
        for paragraph_index, first in enumerate(review):
            generic_replaced = False
            for generic_subject in generic_subjects[language]:
                prefix = generic_subject + " "
                if first.casefold().startswith(prefix.casefold()):
                    first = replace_subject(first[len(prefix):], language)
                    generic_replaced = True
                    break
            if not generic_replaced:
                for title in title_patterns:
                    located, count = re.subn(
                        rf"^\s*(?:En|Dans|On)\s+{title}\s*,?\s*",
                        "",
                        first,
                        count=1,
                        flags=re.IGNORECASE,
                    ...
                    if count:
                        first = upper_first(located)
                        break
                    predicate, count = re.subn(
                        rf"^\s*{title}\s+",
                        "",
                        first,
                        count=1,
                    ...
                    if count:
                        first = replace_subject(predicate, language)
                        break
            editorial[language]["review"][paragraph_index] = first
    return editorial


def expand_editorial_note(
    record: dict,
    editorial: dict,
    opening_by_language: dict[str, str],
) -> dict:
    """Give every note a distinct contextual opening before its sonic analysis."""
    for language in LANGUAGES:
        review = editorial[language]["review"]
        if not review or review[0] != opening_by_language[language]:
            editorial[language]["review"] = [opening_by_language[language], *review]

    longest = max(
        len(" ".join(editorial[language]["review"]).split())
        for language in LANGUAGES
    )
    editorial["readTime"] = max(1, math.ceil(longest / 180))
    return editorial
...
def main() -> None:
    ...
    card_copy_by_id = load_editorial_card_copy()
    editorial_openings = load_editorial_openings()

    master_ids = {record["id"] for record in master}
    if set(card_copy_by_id) != master_ids:
        missing = sorted(master_ids - set(card_copy_by_id))
        extra = sorted(set(card_copy_by_id) - master_ids)
        raise SystemExit(
            "Editorial card-copy coverage mismatch. "
            f"Missing: {missing or 'none'}; extra: {extra or 'none'}"
        )
    if set(editorial_openings) != master_ids:
        missing = sorted(master_ids - set(editorial_openings))
        extra = sorted(set(editorial_openings) - master_ids)
        raise SystemExit(
            "Editorial-opening coverage mismatch. "
            f"Missing: {missing or 'none'}; extra: {extra or 'none'}"
        )
    ...
    for position, record in enumerate(master):
        ...
        editorial = remove_editorial_colons(record, editorial)
        editorial = replace_leading_title(record, editorial)
        editorial = expand_editorial_note(
            record,
            editorial,
            editorial_openings[record["id"]],
        )
        for language in LANGUAGES:
            editorial[language]["entryPoint"]["reason"] = ""
        card_description = card_copy_by_id[record["id"]]
    ...
    print(json.dumps({"albums": len(albums), "songs": len(songs), "editorials": len(notes)}, ensure_ascii=False, indent=2))
