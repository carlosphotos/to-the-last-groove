...
required_statuses = {"metadata", "cover", "streaming", "editorial"}
for record in records:
    require(set(record["status"]) == required_statuses, f"incomplete status block: {record['id']}")
    try:
        navigation_genre = broad_genre(record["genre"])
    except ValueError as error:
        raise SystemExit(f"ERROR: {record['id']}: {error}") from error
    require(
        navigation_genre in BROAD_GENRES,
        f"invalid navigation genre for {record['id']}: {navigation_genre}",
    )
...
print("OK: 200 records, stable IDs, genre taxonomy, 70/20/10 roles per format, edition references and publication gates validated.")
