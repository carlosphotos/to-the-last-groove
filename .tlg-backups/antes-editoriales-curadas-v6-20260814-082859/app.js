...
const elements = {
  ...
  essentialReason: document.querySelector("#essentialReason"),
...
async function loadCatalog() {
  try {
    const [
      ...
      fetch("data/essential-reasons.json")
    ]);
...
function renderRecognition(record) {
  ...
  const essentialReason =
    record.whyEssential?.[state.language] ||
    record.whyEssential?.es ||
    record.whyEssential ||
    "";

  if (!essentialReason && !recognition?.url) {
  ...
  elements.essentialReason.textContent = essentialReason;
  elements.essentialReason.hidden = !essentialReason;

  if (!recognition?.url) {
    elements.recognitionLink.hidden = true;
    elements.recognitionLink.removeAttribute("href");
  ...
  elements.recognitionSource.textContent = recognition.year
    ? `${recognition.source} · ${recognition.year} ↗`
    : `${recognition.source} ↗`;
  elements.recognitionLink.hidden = false;
...
function renderListeningNote() {
  ...
  elements.listeningNoteEntryTrack.hidden =
    state.current.type === "song" &&
    entryTitle.trim().toLocaleLowerCase() ===
      state.current.title.trim().toLocaleLowerCase();
  ...
  elements.listeningNoteListenLabel.hidden =
    !entryTitle || !hasListeningPlatform;
