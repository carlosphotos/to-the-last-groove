    ...
    for language in LANGUAGES:
        ...
        if record["type"] == "album":
            ...
            followups = {
                ...
                "fr": "À la première écoute, prêtez attention à {cue1}. À la seconde, déplacez votre attention vers {cue2}. Ce changement révèle {cue3} et explique pourquoi le morceau permet plusieurs lectures.",
            }
        ...
        for index, cue in enumerate(cues):
            ...
            prefix = {"es": "En", "en": "On", "fr": "Dans"}[language]
            verb = {"es": "escucha", "en": "listen for", "fr": "écoutez"}[language]
            listen_for.append(f"{prefix} {track}, {verb} {cue}.")
...
def main() -> None:
    ...
    for position, record in enumerate(master):
        ...
        if not editorial:
            editorial = generate_editorial(record, service, sources)
        else:
        ...
        reasons.append({"id": record["id"], "whyEssential": essential_reason(record)})
