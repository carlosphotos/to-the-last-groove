...
async function connectUserProgress(user) {
  ...
  const cleanedProgress = progressApi.useUserCollection(
    user.uid,
    mergedProgress
  );
  await writeRemoteProgress(user, cleanedProgress);
  ...
  unsubscribeFromProgress = firestoreSdk.onSnapshot(
    ...
    (nextSnapshot) => {
      ...
      if (progressSignature(merged) !== progressSignature(local)) {
        cleaned = progressApi.useUserCollection(
          activeUser.uid,
          merged
        );
      }
      if (progressSignature(cleaned) !== progressSignature(incoming)) {
        scheduleRemoteSave(cleaned);
      }
