...
def replace_leading_title(record: dict, editorial: dict) -> dict:
    """Avoid repeating the work title immediately below the modal header."""
    subjects = {
        "es": "El disco" if record["type"] == "album" else "La canción",
        "en": "The record" if record["type"] == "album" else "The song",
        "fr": "L’album" if record["type"] == "album" else "Le morceau",
    }
    title = re.escape(record["title"])
    ...
    for language in LANGUAGES:
        ...
        for pattern in patterns:
            ...
            if count:
                first = replaced
                break
        editorial[language]["review"][0] = first
    return editorial
