...
def apply_canonical_metadata_corrections(selection: dict) -> None:
    """Correct credits/spelling without changing the editorial selection."""
    for record in selection.get("songs", []):
        if record.get("artist") == "Spinetta Jade" and record.get("title") == "Barro tal vez":
            # The 1982 recording on Kamikaze is credited to Luis Alberto Spinetta.
            record["artist"] = "Luis Alberto Spinetta"
        if record.get("artist") == "Césaria Évora":
            record["artist"] = "Cesária Évora"
...
def main() -> None:
    ...
    apply_canonical_metadata_corrections(selection)
    ...
    # Preserve the permanent ID allocated before the corrected artist credit.
    prior_ids.setdefault("song:luis alberto spinetta|barro tal vez", "song-083")
