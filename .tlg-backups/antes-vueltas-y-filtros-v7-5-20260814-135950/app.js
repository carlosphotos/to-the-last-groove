...
function applyTranslations() {
  ...
  elements.staffPick.dataset.shortLabel =
    text.recommendation.essentialShort;
...
function updateAnotherButtonLabel() {
  ...
  if (cycleIsComplete) {
    elements.filterMessage.textContent = formatCountMessage(
      text.recommendation.cycleComplete,
      state.completedCycle.count
    );
  }
}
...
function markRecommendationCycleComplete(filterKey, seenCount) {
  ...
  elements.filterMessage.textContent = formatCountMessage(
    getText().recommendation.cycleComplete,
    seenCount
  );
  updateAnotherButtonLabel();
}
